...
export default class Autoaim extends Phaser.Physics.Arcade.Sprite {
    constructor(scene) {
        super(scene, -50, -50, 'autoaim');

        const { width, height } = scene.scale;
        this.constants = new Constants(width, height);
        this.difficulty = this.scene.levelData.meta.difficulty;

        this.autoaimDuration = 5000; // 5 seconds

        // Add to physics and canvas
        scene.physics.add.existing(this);
        this.setInteractive();
        this.setPosition(width * 0.5, height + 50);
        this.setDisplaySize(width * 0.11, width * 0.11);
    ...
    collisionFunc(p, b) {
        ...
        this.scene.events.emit('autoaim', (this.autoaimDuration /* duration in ms */));
